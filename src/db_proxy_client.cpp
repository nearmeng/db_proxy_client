...
static void on_db_proxy_client_redis_rsp(uint64_t qwCoroID, const char* pBuffer, size_t dwSize)
{
    int32_t nRetCode = 0;
    ...
    if (qwCoroID > 0)
    {
        CCoroStackless* pCoro = CGlobalStacklessMgr::instance().get_coro(qwCoroID);
...
BOOL CDBProxyClient::init(IInitInterface* pInit, BOOL bResume)
{
	int32_t nRetCode = 0;
	...
	nRetCode = pInit->register_msg(db_proxy_client_redis_rsp, on_db_proxy_client_redis_rsp);
	LOG_PROCESS_ERROR(nRetCode);
...
BOOL CDBProxyClient::_command(uint64_t qwCoroID, int32_t nCmdID, const char* pUserData, size_t dwUserDataLen, const char* pcszScript, const char* format, va_list args) 
{
	...
	nRetCode = m_pInit->send_to_dbproxy(db_proxy_client_redis_req, (char*)pMsg, 
		sizeof(DB_PROXY_CLIENT_REDIS_REQ) + nCommandSize, qwCoroID);
    LOG_PROCESS_ERROR(nRetCode);
